...
export default function DetailClub() {
  ...
  if (!club) return <div className="text-center text-[var(--m15-muted)] py-12">Club introuvable.</div>;
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-[var(--m15-muted)]">{club.responsable_nom} {club.responsable_prenoms}</p>
                ...
                <div className="flex items-center gap-2 text-[var(--m15-muted)]">
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {user?.role === "eleve" && !club.est_membre && club.mon_statut !== "en_attente" && (
                  <Button onClick={handleInscrire} disabled={isInscribing} className="bg-yellow-500 hover:bg-yellow-600 text-[var(--m15-white)] font-semibold gap-2">
                ...
                {isResponsable && (
                  <Button onClick={() => navigate(`/clubs/${id}/gerer`)} variant="outline" className="border-[var(--m15-border)] text-[var(--m15-white)] gap-2">
      ...
      {/* Onglets */}
      <div className="flex gap-1 bg-[var(--m15-card)] rounded-xl p-1 border border-[var(--m15-border)] flex-wrap">
        {[
          ...
          <button key={tab.key} onClick={() => setActiveTab(tab.key as any)}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all flex-1 justify-center ${activeTab === tab.key ? "bg-[var(--m15-card2)] text-[var(--m15-white)]" : "text-[var(--m15-muted)] hover:text-[var(--m15-white)]"}`}>
          ...
          {club.description && (
            <Card className="bg-[var(--m15-card)] border-[var(--m15-border)]">
              <CardContent className="p-5">
                <p className="text-[var(--m15-white)] leading-relaxed">{club.description}</p>
              </CardContent>
            </Card>
          )}
                ...
                {membres.slice(0, 8).map(m => (
                  <div key={m.id} className="flex items-center gap-3">
                    {m.eleve_photo ? (
                      ...
                      <div className="h-8 w-8 rounded-full bg-[var(--m15-card2)] flex items-center justify-center text-xs text-[var(--m15-white)]">
                        {(m.eleve_nom ?? "?")[0]}
                      </div>
                    )}
                    <div className="flex-1">
                      <p className="text-[var(--m15-white)] text-sm">{m.eleve_nom} {m.eleve_prenoms}</p>
                      {m.classe_nom && <p className="text-[var(--m15-muted)] text-xs">{m.classe_nom}</p>}
                    </div>
                    <Badge variant="outline" className={ROLE_COLORS[m.role_membre] ?? ""}>{m.role_membre}</Badge>
                  </div>
                ))}
                {membres.length > 8 && <p className="text-[var(--m15-muted)] text-sm text-center">+{membres.length - 8} autres membres</p>}
              </CardContent>
            </Card>
          )}
                ...
                {((club.distinctions_recentes as any[]) ?? []).map((d: any) => (
                  <div key={d.id} className="flex items-start gap-3 p-3 bg-yellow-500/10 border border-yellow-500/20 rounded-lg">
                    ...
                    <div>
                      ...
                      <p className="text-[var(--m15-muted)] text-xs">{d.eleve_nom} {d.eleve_prenoms} · {new Date(d.date_obtention).toLocaleDateString("fr-FR")}</p>
                    </div>
                  </div>
                ))}
              </CardContent>
        ...
        <div className="space-y-3">
          {isResponsable && (
            <div className="flex justify-end">
              <Button onClick={() => navigate(`/clubs/${id}/nouvelle-activite`)} className="bg-yellow-500 hover:bg-yellow-600 text-[var(--m15-white)] font-semibold gap-2">
          ...
          {activites.length === 0 ? (
            <div className="text-center text-[var(--m15-muted)] py-12">
              <Calendar className="h-10 w-10 mx-auto mb-2 text-[var(--m15-muted)]" />
              <p>Aucune activité planifiée</p>
            </div>
          ) : (
            activites.map(a => (
              <Card key={a.id} className="bg-[var(--m15-card)] border-[var(--m15-border)] hover:border-[var(--m15-border)] transition-colors cursor-pointer"
                ...
                <CardContent className="p-4 flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    {a.lieu && <p className="text-[var(--m15-muted)] text-xs">{a.lieu}</p>}
                  </div>
                  <div className="flex flex-col items-end gap-1 shrink-0">
                    <Badge variant="outline" className={STATUT_ACTIVITE_COLORS[a.statut] ?? ""}>{a.statut}</Badge>
                    <Badge variant="outline" className="text-[var(--m15-muted)] border-[var(--m15-border)] text-xs">{TYPE_ACTIVITE_LABELS[a.type] ?? a.type}</Badge>
                  </div>
                </CardContent>
              </Card>
            ))
      ...
      {activeTab === "membres" && (
        <div className="space-y-5">
          ...
          <Card className="bg-[var(--m15-card)] border-[var(--m15-border)]">
            ...
            <CardContent className="space-y-2">
              {membres.length === 0 ? (
                <p className="text-[var(--m15-muted)] text-sm text-center py-4">Aucun membre accepté</p>
              ) : membres.map(m => (
                <div key={m.id} className="flex items-center gap-3 p-2 rounded-lg hover:bg-[var(--elevate-2)] transition-colors">
                  {m.eleve_photo ? (
                    <img src={m.eleve_photo} alt="" className="h-9 w-9 rounded-full object-cover" />
                  ) : (
                    <div className="h-9 w-9 rounded-full bg-[var(--m15-card2)] flex items-center justify-center text-xs text-[var(--m15-white)]">
                      {(m.eleve_nom ?? "?")[0]}
                    </div>
                  )}
                  <div className="flex-1">
