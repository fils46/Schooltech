    ...
    <div className="min-h-screen bg-[#0A1628] text-white">
      ...
      <div className="bg-gradient-to-br from-[#111E35] to-[#0A1628] border-b border-[rgba(0,201,167,0.15)] px-6 py-8">
        <div className="max-w-5xl mx-auto">
          <h1 className="font-syne text-3xl font-bold text-white mb-1">Bibliothèque numérique</h1>
          <p className="text-[#8B9DC3] mb-6">Manuels, fiches de cours, exercices et documents officiels</p>

          <div className="relative mb-4">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 h-5 w-5 text-[#8B9DC3]" />
            <Input
              value={search}
              onChange={(e) => { setSearch(e.target.value); setPage(1); }}
              placeholder="Rechercher un manuel, une fiche de cours, un exercice…"
              className="pl-12 h-12 bg-[#1a2a44] border-[rgba(0,201,167,0.25)] text-white placeholder:text-[#8B9DC3] text-base focus-visible:ring-[#00C9A7] rounded-xl"
            />
            {search && (
              <button onClick={() => setSearch("")} className="absolute right-4 top-1/2 -translate-y-1/2 text-[#8B9DC3] hover:text-white">
                <X className="h-4 w-4" />
              </button>
            )}
          </div>
              ...
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-medium border transition-all ${
                  activeType === t
                    ...
                    : "bg-transparent border-[rgba(0,201,167,0.2)] text-[#8B9DC3] hover:border-[#00C9A7] hover:text-white"
                }`}
              >
                {t !== "tous" && TYPE_ICONS[t]}
        ...
        <div className={`${showFilters ? "block" : "hidden"} md:block w-56 shrink-0`}>
          <div className="bg-[#111E35] border border-[rgba(0,201,167,0.15)] rounded-xl p-4 sticky top-6">
            <div className="flex items-center justify-between mb-4">
              <span className="font-semibold text-sm text-white">Filtres avancés</span>
              <button onClick={resetFilters} className="text-xs text-[#00C9A7] hover:underline">Réinitialiser</button>
            </div>

            {/* Niveaux */}
            <div className="mb-4">
              <p className="text-xs text-[#8B9DC3] uppercase tracking-wide mb-2">Niveau</p>
              <div className="space-y-1.5">
                {NIVEAUX.map((n) => (
                  <div key={n} className="flex items-center gap-2">
                    ...
                    />
                    <Label htmlFor={`niv-${n}`} className="text-sm text-[#8B9DC3] cursor-pointer">{n}</Label>
                  </div>
                ))}
              </div>
            </div>
            ...
            {/* Langue */}
            <div>
              <p className="text-xs text-[#8B9DC3] uppercase tracking-wide mb-2">Langue</p>
              <div className="space-y-1.5">
                {LANGUES.map((l) => (
                  <div key={l.value} className="flex items-center gap-2">
                    <Checkbox
                      ...
                      className="border-[rgba(0,201,167,0.3)] data-[state=checked]:bg-[#00C9A7] data-[state=checked]:border-[#00C9A7]"
                    />
                    <Label htmlFor={`lang-${l.value}`} className="text-sm text-[#8B9DC3] cursor-pointer">{l.label}</Label>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center justify-between mb-4 gap-3">
            <div className="flex items-center gap-2">
              <button
                className="md:hidden flex items-center gap-1.5 text-sm text-[#8B9DC3] border border-[rgba(0,201,167,0.2)] rounded-lg px-3 py-1.5 hover:text-white"
            ...
            <div className="flex items-center gap-2">
              <Select value={tri} onValueChange={(v) => { setTri(v); setPage(1); }}>
                <SelectTrigger className="w-44 bg-[#111E35] border-[rgba(0,201,167,0.2)] text-white text-sm h-8">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent className="bg-[#111E35] border-[rgba(0,201,167,0.2)]">
                  <SelectItem value="date" className="text-white focus:bg-[#1a2a44]">Plus récent</SelectItem>
                  <SelectItem value="consultations" className="text-white focus:bg-[#1a2a44]">Plus consulté</SelectItem>
                  <SelectItem value="telechargements" className="text-white focus:bg-[#1a2a44]">Plus téléchargé</SelectItem>
                </SelectContent>
              </Select>
              <button
                onClick={() => setViewMode("grid")}
                className={`p-1.5 rounded ${viewMode === "grid" ? "text-[#00C9A7]" : "text-[#8B9DC3]"}`}
              >
                <Grid3X3 className="h-4 w-4" />
              </button>
              <button
                onClick={() => setViewMode("list")}
                className={`p-1.5 rounded ${viewMode === "list" ? "text-[#00C9A7]" : "text-[#8B9DC3]"}`}
              >
                    ...
                    <CardContent className="p-4">
                      ...
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        <button
                          ...
                          className={`shrink-0 transition-colors ${r.est_favori ? "text-yellow-400" : "text-[#8B9DC3] hover:text-yellow-400"}`}
                        >
                          {r.est_favori ? <Star className="h-4 w-4 fill-current" /> : <Star className="h-4 w-4" />}
                        </button>
                      </div>

                      <p className="font-semibold text-white text-sm leading-snug mb-1 line-clamp-2">{r.titre}</p>
              ...
              {niveauFiltered.map((r) => {
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-semibold text-white text-sm truncate">{r.titre}</p>
                      <p className="text-xs text-[#8B9DC3] truncate">{r.auteur_nom ?? r.auteur ?? ""} · {(r.niveau ?? []).join(", ")}</p>
                    </div>
                    <Badge className={`text-xs border shrink-0 ${TYPE_COLORS[type] ?? TYPE_COLORS["autre"]}`}>
                      {TYPE_LABELS[type]}
                    </Badge>
                    <div className="flex items-center gap-3 text-xs text-[#8B9DC3] shrink-0">
                      <span className="flex items-center gap-1"><Eye className="h-3 w-3" />{r.nb_consultations ?? 0}</span>
                      <span className="flex items-center gap-1"><Download className="h-3 w-3" />{r.nb_telechargements ?? 0}</span>
                    </div>
                    <button
                      onClick={(e) => { e.stopPropagation(); r.id && toggleFavori.mutate({ id: r.id }); }}
                      className={`shrink-0 ${r.est_favori ? "text-yellow-400" : "text-[#8B9DC3] hover:text-yellow-400"}`}
                    >
          ...
          {totalPages > 1 && (
            <div className="flex items-center justify-center gap-2 mt-8">
              <Button
                ...
                className="border-[rgba(0,201,167,0.2)] text-white hover:bg-[#111E35]"
              >
                ← Précédent
              </Button>
              <span className="text-sm text-[#8B9DC3]">Page {page} / {totalPages}</span>
              <Button
                variant="outline"
                size="sm"
