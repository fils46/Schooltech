...
export default function MonProfil() {
  ...
  const handlePhotoChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 3 * 1024 * 1024) {
      toast({ title: "Photo trop grande", description: "La photo ne doit pas dépasser 3 Mo.", variant: "destructive" });
      e.target.value = "";
      return;
    }
      ...
      <div className="rounded-2xl p-6 space-y-6" style={{ background: "var(--m15-card)", border: "1px solid var(--m15-border)" }}>
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-2">
            ...
            <p className="text-xs" style={{ color: "var(--m15-muted)" }}>PNG, JPG ou WebP · 3 Mo max</p>
          </div>
        </div>
